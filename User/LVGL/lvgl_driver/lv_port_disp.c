...
static void disp_flush(lv_disp_drv_t *disp_drv, const lv_area_t *area, lv_color_t *color_p)
{
    ...
    #if 1
    DMA_Fill(area->x1,area->y1,area->x2,area->y2,(uint8_t *)color_p);
    #else
    uint16_t i, j;
	LCD_Address_Set(area->x1, area->y1, area->x2-1, area->y2-1); //设置显示范围
	for (i = area->y1; i < area->y2; i++)
	{
		for (j = area->x1; j < area->x2; j++)
		{
			LCD_WR_DATA((u16)(*(color_p+i+j)));
		}
	}
...
{
	if (hspi->Instance == hspi1.Instance && t ==0)
	{
